...
/**
 * TaskManager manages all tasks that are to be performed by the application.
 * 
 * @class TaskManager
 * @singleton
 */
var TaskManager = {
	 ...
	 * 
 	 ...
 	 * @return {Number} The identifier for this task owner
	 */
	register: function(name, owner) {
		var ownerId = _ownerCounter++;
	 ...
	 * 
	 * @param {Number} ownerId Id of the owner this task is to be added to
	 * @param {Object|Array"} task A task description, or an array of thereof
	 */
	 ...
	 * 
	 * @param {String|Number} ownerId of the owner that contains the required task(s)
	 ...
	 * 
	 * @param {Number} [activeOwnerId] If passed, then this owner will be activated first (if it was inactive)
	 */
...
/**
 * @property {Number} _activeInstance The ID of the last registered or activated task owner
 * @private
 */
...
/**
 * @property {Array} _priorityStack Stack of dictionaries that define request that are to be handled asap.
 * 
...
/**
 * @property {Boolean} _running True if running, false otherwise
 * @private
 */
	_running = false,
 ...
 * @private
 * 
 * The core of TaskManager. This function checks owners (in order of priority)
 * for pending tasks and executes the first returned.
 */
 ...
 * the normal operation of this TaskManager.
 * @private
 * 
 * @param {String} ownerId The id of the owner that owns the task
 * @param {String|Number} taskId The id of the task that is to be performed
 */
 ...
 * @private
 * 
 * This is a special function that is not called directly. A task is bound to it as its content,
 * and it will wrap a task.
 * 
